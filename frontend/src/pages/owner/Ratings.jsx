...
const OwnerRatings = () => {
  ...
  const loadMine = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const prev = getCachedData('owner_ratings') || {}
      setCachedData('owner_ratings', {
        ...prev,
        received: rec,
        given: giv,
  ...
  useEffect(() => {
    const run = async () => {
      const cached = getCachedData('owner_ratings')
      if (tab === 'mine') {
  ...
  const submitRating = async (contract, score, reviewText) => {
    ...
    try {
      ...
      toast.success(t('ownerRatingSubmitted'))
                      ...
                      <div className="flex gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {c.duration} {t('days')} · {t('startLabel2')}{' '}
                            {fmtDate(c.startDate)} · ₹
